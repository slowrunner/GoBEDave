...
def getClockFreq():
    ...
    res = int(res.split("=")[1])
    if (res < 1000000000):
        res = "{:.0f} MHz".format(res/1000000)
    else: res = '{:.2f} GHz'.format(res/1000000000.0)
    return res
...
def printStatus():
  ...
  print("\n********* GoBEDave safetyShutdown STATUS *****")
  ...
  try:
    while True:
        ...
        if (batteryLowCount > 3):
          ...
          egpg.reset_all()
          time.sleep(1)
          os.system("/home/pi/GoBEDave/logMaintenance.py 'SAFETY SHUTDOWN - BATTERY LOW'")
          str_log_voltages = "/home/pi/GoBEDave/logMaintenance.py " + " '" + battery.voltages_string(egpg) +"' "
