...
# file:  gobedavetDataJson.py
#
# Serialize data values to /home/pi/rosbot-on-gopigo3/rosbotData.json
#
# Methods:
...
#
import sys
sys.path.append('/home/pi/GoBEDave/plib')
...
def saveData(dataname, datavalue, logit=False):
    ...
    with DataLock:         # prevents two different saveData() at same time
        ...
        try:
            ...
            with open('/home/pi/GoBEDave/gobedaveData.json', 'w') as outfile:
...
def getData(dataname=None):
    ...
    try:
        with open('/home/pi/GoBEDave/gobedaveData.json', 'r') as infile:
...
def printData():
    print("gobedaveData.json contents:")
    lData = getData()
    if lData != None:
        for i in lData:
            print("  ",i," : ",lData[i])
    else:
        print("gobedaveData.json contains no data")
...
def main():
    ...
    if 'chargeCycles' in lData:
        print("removing chargeCycles from gobedaveData.json")
